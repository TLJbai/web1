...
public class IndexController {
    ...
    public String indexPage(HttpServletRequest request) {
        List<NewBeeMallIndexCategoryVO> categories = mallGoodsCategoryService.getCategoriesForIndex();
        if (CollectionUtils.isEmpty(categories)) {
            return "error/error_5xx";
        }
        //轮播图
        List<NewBeeMallIndexCarouselVO> carousels = mallCarouselService.getCarouselsFotIndex(Constants.INDEX_CAROUSEL_NUMBER);
        //三大栏目：热销栏目、最新栏目、推荐栏目
        ...
        request.setAttribute("newGoodses",newGoodses);//热门数码产品
